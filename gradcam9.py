...
def compute_saliency(model, guided_model, img_path, layer_name='block5_conv3', cls=-1, visualize=True, save=True, filename='', num=0):
    ...
    if save:
        ...
        Path(filename + '/gradcam/').mkdir(parents=True, exist_ok=True)
        Path(filename + '/guided_backprop/').mkdir(parents=True, exist_ok=True)
        Path(filename + '/guided_gradcam/').mkdir(parents=True, exist_ok=True)
        cv2.imwrite(os.path.join(filename + '/gradcam/', str(num) + '.jpg'), np.uint8(jetcam))
        cv2.imwrite(os.path.join(filename + '/guided_backprop/', str(num) + '.jpg'), deprocess_image(gb[0]))
        cv2.imwrite(os.path.join(filename + '/guided_gradcam/', str(num) + '.jpg'), deprocess_image(guided_gradcam[0]))
    ...
    with alive_bar(x.shape[0]) as bar:
        for img in x:
            ...
            gradcam, gb, guided_gradcam = compute_saliency(model, guided_model, layer_name='block5_conv3',
                                                img_path=img, cls=-1, visualize=False, save=True, filename=filename,num=i)
            bar()
